...
import { ChangeLanguege, ReadLanguageSet, TranslateTokey } from './i18n'
    ...
    setup(){
        ...
        const initSet = () => {
            if(localStorage.getItem('lang') || localStorage.getItem('language')){
                let language = localStorage.getItem('lang') || localStorage.getItem('language')
                ChangeLanguege(language)
                document.documentElement.lang = language
            }
            initLangConfig(ReadLanguageSet())
        ...
        const loginVerify = () => {
            ...
            .then(res => {
                ...
                MessagePlugin.success(TranslateTokey('welcomeBack') + '！' + res.user.nickname)
            })
            .catch(() => {
                MessagePlugin.info(TranslateTokey('loginFailure'))
                user.status = 'unlog'
